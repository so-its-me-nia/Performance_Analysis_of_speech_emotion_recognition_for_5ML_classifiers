...
from create_csv import write_emodb_csv, write_tess_ravdess_csv, write_custom_csv, write_surrey_csv
...
class EmotionRecognizer:
    ...
    def __init__(self, model=None, **kwargs):
        ...
        """
        # emotions
        self.emotions = kwargs.get("emotions", ['angry', 'sad', 'neutral', 'fear', 'happy'])
        ...
        self.surrey_db = kwargs.get("surrey_db", True)
        # self.crema_db = kwargs.get("crema_db", True)


        if not self.tess_ravdess and not self.emodb and not self.custom_db and not self.surrey_db:
        ...
        self.surrey_name = kwargs.get("surrey_name", "surrey.csv")
        # self.crema_name = kwargs.get("crema_name", "crema.csv")
    ...
    def _set_metadata_filenames(self):
        ...
        if self.surrey_db:
            train_desc_files.append(f"train_{self.surrey_name}")
            test_desc_files.append(f"test_{self.surrey_name}")
        # if self.crema_db:
        #     train_desc_files.append(f"train_{self.crema_name}")
        #     test_desc_files.append(f"test_{self.crema_name}")
    ...
    def write_csv(self):
        ...
        for train_csv_file, test_csv_file in zip(self.train_desc_files, self.test_desc_files):
            ...
            if self.emodb_name in train_csv_file:
                ...
                write_custom_csv(self.emotions, train_name=train_csv_file, test_name=test_csv_file, verbose=self.verbose)
                if self.verbose:
                    print("[+] Generated Custom DB CSV File")
            elif self.surrey_name in train_csv_file:
                write_surrey_csv(self.emotions, train_name=train_csv_file, test_name=test_csv_file, verbose=self.verbose)
                if self.verbose:
                    print("[+] Generated Surrey DB CSV File")
            # elif self.crema_name in train_csv_file:
            #     write_crema_csv(self.emotions, train_name=train_csv_file, test_name=test_csv_file, verbose=self.verbose)
            #     if self.verbose:
            #         print("[+] Generated CREMA DB CSV File")
...
# 11111111111111111111111111111111
        for estimator, params, cv_score in estimators:
            ...
            detector = EmotionRecognizer(estimator, emotions=self.emotions, tess_ravdess=self.tess_ravdess,
                                        emodb=self.emodb, custom_db=self.custom_db, surrey_db=self.surrey_db, classification=self.classification,
